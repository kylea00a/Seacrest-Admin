...
import type { InventoryFlowDayRow, InventoryRtsInEntry, InventorySupplyEntry } from "@/data/admin/types";
...
export function calcFlowEnding(
  ...
  for (const p of productNames) {
    ending[p] = (beginning[p] ?? 0) + (delivery[p] ?? 0) + (rtsIn[p] ?? 0) - (out[p] ?? 0);
  }
  return ending;
}
...
export async function syncInventoryFlowDay(date: string): Promise<InventoryFlowDayRow> {
  ...
  const ending = calcFlowEnding(beginning, delivery, rtsIn, out, productNames);
...
/**
 * After a day changes, refresh that day and the next day (next beginning depends on this ending).
 */
...
export function getInventoryFlowRow(date: string): InventoryFlowDayRow | null {
  return loadInventoryFlow().byDate?.[date] ?? null;
}
