...
import { nonChipsRepurchaseAmountFromOrder } from "@/lib/salesReportChipsRepurchase";
...
export default function SeacrestSalesReportPage() {
  ...
  const daily = useMemo(() => {
    ...
    for (const row of rows) {
      ...
      dr.repurchaseTotal += nonChipsRepurchaseAmountFromOrder(rep, productPriceByName);
  ...
  const dailyTotal = (d: DailyRow): number =>
    d.packageAmount + d.subscriptionAmount + d.repurchaseTotal + d.deliveryFee + d.deliveryFeeOthers;
  ...
  const depositTransferForDay = (salesDate: string) => {
    const txs = cashTx.filter((t) => t.kind === "seacrest_sales_deposit" && t.salesDate === salesDate);
  ...
  const undoDepositSalesDay = async (salesDate: string) => {
    ...
    try {
      const res = await fetch("/api/admin/cash", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "undoDepositSeacrestSalesDay", salesDate }),
      });
      const json = (await res.json()) as { ok?: boolean; file?: { accounts?: BankAccount[]; transactions?: CashTransaction[] }; error?: string };
      if (!res.ok) throw new Error(json.error ?? `Failed (${res.status})`);
      setCashAccounts(json.file?.accounts ?? cashAccounts);
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_420px]">
      <div className="admin-card min-w-0">
        <h1 className="admin-title">Seacrest Sales Report</h1>
        <div className="admin-muted">Same as Sales Report but excludes repurchase chips (see JJ Sales Report for chips).</div>
            ...
            <table className="min-w-[980px] text-xs">
              <thead className="bg-black/30 text-zinc-300">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right whitespace-nowrap">Repurchase (excl. chips)</th>
      ...
      <div className="admin-card">
        ...
        <div className="mt-6 border-t border-white/10 pt-5">
          ...
          <div className="mt-1 text-xs text-zinc-400">
            Bank-to-bank transfer per sales day (
            <span className="font-mono">seacrest-sales-YYYY-MM-DD</span>). Includes Delivery fee (Others).
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-zinc-950 p-4 shadow-2xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-sm font-semibold text-white">Confirm Seacrest Sales Transfer</div>
                <div className="admin-muted mt-1 text-xs">
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-3 text-xs text-zinc-300">
              ...
              <span className="font-mono text-zinc-100">seacrest-sales-{depositDialog.salesDate}</span>).
            </div>
